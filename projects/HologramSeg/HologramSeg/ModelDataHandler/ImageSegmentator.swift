...
//  HologramSeg
//
//  Created by Tan Ren Jie on 5/1/21.
//
...
private enum Constants {
    ...
    static let modelFileName = "deeplabv3_dynamic_range_quant"
...
class ImageSegmentator {
    ...
    /// Tensorflow Lite `Interpretor` object for performing inference on a given model
    ...
    /// Label list contains name of all classes the model can recognize
    ...
    private static func loadLabelList() -> [String]? {
        guard let labelListPath = Bundle.main.path(forResource: Constants.labelsFileName, ofType: Constants.labelsFileExtension)
        else {
            return nil
        }
    ...
    static func newInstance(completion: @escaping ((Result<ImageSegmentator>) -> Void)) {
        /// Create a dispatch queue to ensure all operations on the Interpreter will run serially.
        let tfLiteQueue = DispatchQueue(label: "me.renjietan.hologram_seg")
        
        /// Run initialization in background thread tp avoid UI freeze.
        tfLiteQueue.async {
            /// Contruct the path to the model file
            guard let modelPath = Bundle.main.path(forResource: Constants.modelFileName, ofType: Constants.modelFileExtension) else {
                ...
                DispatchQueue.main.async {
                    completion(
                        .error(
                            InitializationError.invalidModel("\(Constants.modelFileName).\(Constants.modelFileExtension)")
                        )
                    )
                }
                return
            ...
            guard let labelList = loadLabelList() else {
                print("Failed to load the label list file wiht name: \(Constants.labelsFileName).\(Constants.labelsFileExtension)")
                DispatchQueue.main.async {
                    completion(.error(InitializationError.invalidLabelList("\(Constants.labelsFileName).\(Constants.labelsFileExtension)")))
                }
                return
            }
...
//            delegates = [MetalDelegate()]
            ...
            do {
                ...
                /// Create an ImageSegementator instance and return
    ...
    func runSegmentation(_ image:UIImage, completion: @escaping ((Result<SegmentationResult>) -> Void)) {
        tfLiteQueue.async {
            ...
            do {
                /// Preprocessing: Resize the input UIImage to match TF Lite model input shape.
                guard
                    let rgbData = image.scaledData(
                        with: CGSize(width: self.inputImageWidth, height: self.inputImageHeight), byteCount: self.inputImageWidth * self.inputImageHeight * self.inputPixelSize, isQuantized: false
                    ) else {
                    DispatchQueue.main.async {
            ...
            /// Postprocessing: Find the class with the highest confidence for each pixel
            let parsedOutput = self.parseOutputTensor(outputTensor: outputTensor)
            
            /// Calculate postprocessing time.
            /// Note: You may find the postprocessing very slow if you run the sample app with debug build
            /// You will see significant speed up if you rerun using Release build or change optimization level in the project Build's Settings to the same value with Release build
            var now = Date()
            ...
            guard
                let resultImage = ImageSegmentator.imageFromSRGBColorArray(
                    pixels: parsedOutput.segmentationImagePixels, width: self.inputImageWidth, height: self.inputImageHeight
                ),
            ...
            /// Construct a dictionary of classes found in the image and each class's color used in visualization
            ...
            /// Create a representative object that contains the segmentation results
            let result = SegmentationResult(array: parsedOutput.segmentationMap, resultImage: resultImage, overlayImage: overlayImage, preprocessingTime: preprocessingTime, inferenceTime: inferenceTime, postProcessingTime: postprocessingTime, visualizationTime: visualizationTime, colorLegend: colorLegend)
    ...
    /// Post-processing Convert Tensorflow Lite output tensor to segmentation map and its color representation
    private func parseOutputTensor(outputTensor: Tensor) -> (segmentationMap: [[Int]], segmentationImagePixels: [UInt32], classList: Set<Int>){
        
        /// Initialize the variables to store postprocessing result
    ...
    private static func imageFromSRGBColorArray(pixels: [UInt32], width: Int, height: Int) -> UIImage? {
        ...
        /// Convery array of pixels to a CGImage instance
        let cgImage = data.withUnsafeMutableBytes {
            (ptr) -> CGImage in let ctx = CGContext(data: ptr.baseAddress, width: width, height: height, bitsPerComponent: 8, bytesPerRow: MemoryLayout<UInt32>.size * width, space: CGColorSpace(name: CGColorSpace.sRGB)!, bitmapInfo: CGBitmapInfo.byteOrder32Little.rawValue
